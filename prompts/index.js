  ...
  function fileSelectionSettingPrompt() {
    return inquirer.prompt([
      ...
      },
      {
        ...
        choices: () => [
          {
            name: `Year\t ${chalk.gray(
              format(new Date(), "yyyy", new Date())
            )}`,
            value: "yyyy",
            short: "Year",
          },
          {
            name: `Month\t ${chalk.gray(
              format(new Date(), "yyyy-MM", new Date())
            )}`,
            value: "yyyy-MM",
            short: "Month",
          },
          {
            name: `Day\t ${chalk.gray(
              format(new Date(), "yyyy-MM-dd", new Date())
            )}`,
            value: "yyyy-MM-dd",
            short: "Day",
          },
      ...
      },
      {
        ...
        message: ({ pathToDir, filter, dateFormat }) =>
          `Scan all files from ${chalk.yellow(
            path.normalize(pathToDir)
          )}, filtered by ${chalk.yellow(
            filter
          )} and group them like ${chalk.yellow(
